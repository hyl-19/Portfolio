...
Node * InfixToPreFix( string in ) {
// 中序轉後序 
	
	Node * stack = NULL ; // 存運算子 
	Node * output = NULL ; // 存整體的資訊 
	char value[100] = {0} ; // 存數字使用的 
	string str ; // 存數字使用的 
	int index = 0 ; // 紀錄value的位置 
	
	for ( int i = in.size() - 1 ; i >= 0 ; i-- ) { // 拿字串裡的字元一個一個比對 
		
		if ( '0' <= in[i] && in[i] <= '9' ) { // 是數字 
				value[index] = in[i] ;
				index++ ;
			if ( !('0' <= in[i-1] && in[i-1] <= '9') || i == 0 ) { // 數字下一個是運算子 
				str = value ; // 字元陣列轉字串 
				reverse(str.begin(), str.end() ) ;
				output = Push(str, output) ; // 存入stack 
				memset(value, 0, 100) ; // 清空value 
				index = 0 ; // 重新計算 
			} // if
		} // if
		...
		// 是運算子	
			if ( in[i] == ')' ) stack = Push(")", stack) ; // 右括弧先存入 
			else if ( in[i] == '(' ) { // 括號內先做 
				while( GetTopData(stack) != ")" ) { // 遇到左括弧前都丟入output
					output = Push(GetTopData(stack), output) ;
					stack = Pop(stack) ;
				} // while
				
				stack = Pop(stack); // 刪除已存入的運算子 
			} // else if
			else { 
				str = SetOper(in[i]) ; // 轉換型別 
				if ( isEmpty(stack) || OperPriority(str) >= OperPriority(GetTopData(stack)) ) 
				// 若stack為空的或運算子的優先順序小於stack of top data 
					stack = Push(str, stack) ; // 存入stack
				else {
				// 若不是空的或運算子的優先順序大於stack of top data
					while ( !isEmpty(stack) && OperPriority(str) < OperPriority(GetTopData(stack)) ) {
						output = Push(GetTopData(stack), output) ; // 存入output  
						stack = Pop(stack) ; // 刪除已加入output的運算子
					} // while
					
					stack = Push(str, stack); // 存入output 
				} // else
			} // else
		} // else if
...
int Calculate1( int num1, int num2, string op ) {
// 回傳運算後的數字 
...
int CalculatePrefix( Node * list , int & num ) {
// 計算後序
	
	string value ; // 紀錄list裡的data 
	int stack[10] ; // 存stack裡的數字 
	int top = 0 ; // 紀錄stack的位置
	
	while ( !isEmpty(list) ) {
		
		value = GetTopData(list) ; // 取出data 
		
		if ( isDigit(value) )
			stack[top++] = atoi(value.c_str()) ; // 存入數字 
		else {
			int num1 = stack[--top] ; // 取出第一個數字 
			int num2 = stack[--top] ; // 取出第二個數字 
			
			// 把計算好的數字丟到取出數字位置的前面 
			if ( value == "+" ) stack[top++] = Calculate1(num1, num2, value) ;
			else if (value == "-" ) stack[top++] = Calculate1(num1, num2, value) ;
			else if (value == "*" ) stack[top++] = Calculate1(num1, num2, value) ;
			else if (value == "/" ) {
				if ( num2 == 0 ) { // 除數為0不正確 
					num = num2 ;
				...
				stack[top++] = Calculate1(num1, num2, value) ; 
			} // else if 
		} // else
...
// 任務一 
	...
	if ( FindError(input, error.type, error.ch, error.par, error.oper) ) // 尋找error 
		...
		Print(postfix) ; // 印出後續 
		int total = CalculatePrefix(postfix, errorNum) ; // 計算答案 
...
void mission2() {
	...
	while ( !isEmpty(prefix) ) {
		list = Push(prefix->data, list) ;
		prefix = Pop(prefix) ;
	} // while
	
	total = CalculatePrefix(list, errorNum) ;
	if ( errorNum == 0 ) { // 除數為0 
			cout << "### Error: Divided by ZERO! ###" << endl ;
			cout << "### It cannot be successfully evaluated! ###" << endl ;
			prefix = Clear(prefix) ; // 釋放記憶體 
		} // if
		else cout << "Answer: " << total << endl ;
